...
JsonlProgressCallback = Callable[[int, int], None]
...
def extract_element(
    ...
    *,
    ...
    # Extract geometry once, then derive all secondary geometry from that pass.
    geometry_data = get_element_geometry_data(element, geom_settings)
    centroid = geometry_data.centroid
    bbox = geometry_data.bbox
    mesh_vertices = geometry_data.vertices
    mesh_faces = geometry_data.faces
...
def convert_ifc_to_jsonl(
    ifc_path: Path,
    out_path: Path,
    *,
    progress_callback: JsonlProgressCallback | None = None,
) -> int:
    ...
    def emit_progress(processed: int) -> None:
        ...
        if processed not in {0, total} and progress_percent == last_progress_percent:
            return
    ...
    with out_path.open("w", encoding="utf-8") as fh:
        for index, elem in enumerate(unique, 1):
            try:
